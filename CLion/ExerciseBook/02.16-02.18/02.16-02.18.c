...
//    Algo_2_16(&la, &lb, 1, 3, 3);
//    printf("█ la = ");
//    Output(la); //1 3 9 11 13 15 17 19
//    printf("█ lb = "); // 2 4 5 99  7 6  8 10 12 14 16 18 20
//    Output(lb);
//    PressEnterToContinue(debug);
//
    printf("████████ 题 2.18 验证...\n");
...
Status Algo_2_16(LinkList* la, LinkList* lb, int i, int j, int len) {
    ...
    // 确定p的位置
    k = 1;
//    测试将la给q q在la最后连接lb 发现la也连接了 即 变成了la = la +lb
//    q = *la;
//    while (q->next != NULL) {
//        q = q->next;
//    }
//    q->next = *lb;
    ...
    if(i == 1) {
        ...
        // 查找la中第i-1个结点，且保证它后继存在 k < i-1  i=5 k < 4
    ...
    // p为NULL q从该链表开头开始， 否则 q从p的下一个节点开始
    if(p == NULL) {
        q = *la;
    } else {
        q = p->next;
